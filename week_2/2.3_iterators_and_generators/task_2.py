...
def primes():
    # число и факториал предыдущего числа
    i, f = 2, 1
    while True:
        # проверяем на простоту по теореме Вильсона через факториал
        if (f + 1) % i == 0:
            yield i
        # сначала пересчитываем факториал для текущего числа, затем увеличиваем число
        f, i = f * i, i + 1
